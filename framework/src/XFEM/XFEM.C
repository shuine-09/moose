...
XFEM::store_crack_tip_origin_and_direction()
{
  ...
  for (sit = CrackTipElements.begin(); sit != CrackTipElements.end(); ++sit)
  {
    if (_mesh->mesh_dimension() == 2){
      ...
      std::cout << "WJ : crack tip id = " << cts_id << std::endl;
...
XFEM::correct_crack_extension_angle(const Elem * elem, EFAelement2D * CEMElem, EFAedge * orig_edge, Point normal, Point crack_tip_origin, Point crack_tip_direction, Real & distance_keep, unsigned int & edge_id_keep, Point & normal_keep)
{
  ...
  Real cos_45 = std::cos(80.0/180.0*3.14159);
  Real sin_45 = std::sin(80.0/180.0*3.14159);
  ...
  for (unsigned int i = 0; i < nsides; ++i)
  {
    if (!orig_edge->isPartialOverlap(*CEMElem->get_edge(i)))
    {
      ...
      if(std::abs(angle_edge1_normal) > std::abs(angle_min) && (edge1_to_tip*crack_tip_direction) > std::cos(80.0/180.0*3.14159))
      {
        edge_id_keep = i;
        distance_keep = 0.05;
      ...
      else if (std::abs(angle_edge2_normal) > std::abs(angle_min) && (edge2_to_tip*crack_tip_direction) > std::cos(80.0/180.0*3.14159))
      {
        edge_id_keep = i;
        distance_keep = 0.95;
      ...
      {
        if(std::abs(left_angle_normal*normal) > std::abs(angle_min) && (edge1_to_tip*crack_tip_direction) > std::cos(80.0/180.0*3.14159))
        {
          edge_id_keep = i;
      ...
      {
        if(std::abs(right_angle_normal*normal) > std::abs(angle_min) && (edge2_to_tip*crack_tip_direction) > std::cos(80.0/180.0*3.14159))
        {
          edge_id_keep = i;
          distance_keep = distance;
      ...
      {
        if(std::abs(crack_direction_normal*normal) > std::abs(angle_min) && (crack_tip_direction*crack_tip_direction) > std::cos(80.0/180.0*3.14159))
        {
          edge_id_keep = i;
          distance_keep = distance;
...
XFEM::mark_cut_edges_by_state(Real time)
{
       ...
       pmeit != _state_marked_elems.end(); ++pmeit)
  {
    ...
    if (!is_elem_at_crack_tip(elem)) // crack tip element's crack intiation
    {
      ...
      {
        std::cout << "WJ : fond boundary crack initiation " << std::endl;
        ...
        {
          std::cout << "WJ : marked boundary virtual crack tip direction " << std::endl;
    ...
    for (unsigned int i = 0; i < nsides; ++i)
    {
      if (!orig_edge->isPartialOverlap(*CEMElem->get_edge(i)))
      {
        std::cout << "WJ : loop nsides " << std::endl;
        edge_ends[0] = get_efa_node_coor(CEMElem->get_edge(i)->get_node(0),CEMElem,elem);
        edge_ends[1] = get_efa_node_coor(CEMElem->get_edge(i)->get_node(1),CEMElem,elem);
        std::cout << "WJ : edge_ends[0] =  " << edge_ends[0] << std::endl;
        std::cout << "WJ : edge_ends[1] =  " << edge_ends[1] << std::endl;
        std::cout << "WJ : crack_tip_origin = " << crack_tip_origin << std::endl;
  ...
  for (pmeit = _elem_crack_origin_direction_map.begin(); pmeit != _elem_crack_origin_direction_map.end(); ++pmeit)
  {
    ...
    if (is_elem_at_crack_tip(elem)) // crack tip element's crack intiation
    {
      ...
      if (ecodm != _elem_crack_origin_direction_map.end()){
        ...
        std::cout << "WJ : crack_tip_origin = " << crack_tip_origin << std::endl;
        std::cout << "WJ : crack_tip_direction = " << crack_tip_direction << std::endl;
