...
#include "SideSetsFromAllElementFaces.h"
...
registerMooseObject("MooseApp", SideSetsFromAllElementFaces);

template <>
InputParameters
validParams<SideSetsFromAllElementFaces>()
{
  ...
  params.addClassDescription("Adds element faces of the given block to the sidesets specified");

  return params;
}

SideSetsFromAllElementFaces::SideSetsFromAllElementFaces(const InputParameters & parameters)
...
SideSetsFromAllElementFaces::generate()
{
  ...
  std::vector<subdomain_id_type> block_ids(blocks.begin(), blocks.end());
    ...
    for (unsigned int side = 0; side < elem->n_sides(); ++side)
    {
      ...
      if (neighbor == remote_elem)
      {
        queries[elem->processor_id()].push_back(std::make_pair(elem->id(), side));
               ...
               (neighbor != nullptr && block_ids[0] == neighbor->subdomain_id()))
      {
