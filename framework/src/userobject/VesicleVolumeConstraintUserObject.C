...
void
VesicleVolumeConstraintUserObject::initialize()
{
  _integral = 0.0;
...
void
VesicleVolumeConstraintUserObject::execute()
{
  //
  // integrate u over the simulation domain
  //
  for (unsigned int qp = 0; qp < _qrule->n_points(); ++qp)
    _integral += _JxW[qp] * _coord[qp] * (0.5 * (1.0 - _u_value[qp]));
}

void
VesicleVolumeConstraintUserObject::executeJacobian(unsigned int jvar)
{
  ...
  if (jvar == _u_var)
  {
    ...
    Real sum = 0.0;
    for (unsigned int qp = 0; qp < _qrule->n_points(); ++qp)
      sum += _JxW[qp] * _coord[qp] * _phi[_j][qp] * (-0.5);
...
void
VesicleVolumeConstraintUserObject::finalize()
{
  gatherSum(_integral);
  if (_fe_problem.currentlyComputingJacobian())
