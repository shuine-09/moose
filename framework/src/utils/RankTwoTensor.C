...
template <typename T>
void RankTwoTensorTempl<T>::calculateEigenvalueAndVector(
    std::array<T, 3> & eval, std::array<std::array<T, 3>, 3> & evec) const
{
  ...
  T max0 = std::max(std::abs(a00), std::abs(a01));
  T max1 = std::max(std::abs(a02), std::abs(a11));
  T max2 = std::max(std::abs(a12), std::abs(a22));
...
template <typename T>
std::array<T, 3>
RankTwoTensorTempl<T>::Multiply(T s, std::array<T, 3> const & U)
{
...
template <typename T>
std::array<T, 3> RankTwoTensorTempl<T>::Subtract(std::array<T, 3> const & U,
...
template <typename T>
std::array<T, 3> RankTwoTensorTempl<T>::Divide(std::array<T, 3> const & U, T s)
{
  T invS = (T)1 / s;
...
template <typename T>
T RankTwoTensorTempl<T>::Dot(std::array<T, 3> const & U, std::array<T, 3> const & V)
{
...
template <typename T>
std::array<T, 3> RankTwoTensorTempl<T>::Cross(std::array<T, 3> const & U,
                                              std::array<T, 3> const & V)
{
  std::array<T, 3> cross = {
...
template <typename T>
void RankTwoTensorTempl<T>::ComputeOrthogonalComplement(std::array<T, 3> const & W,
                                                        std::array<T, 3> & U,
...
template <typename T>
void
RankTwoTensorTempl<T>::ComputeEigenvector0(
...
template <typename T>
void
RankTwoTensorTempl<T>::ComputeEigenvector1(T a00,
                                           T a01,
                                           T a02,
                                           T a11,
                                           T a12,
                                           ...
                                           std::array<T, 3> & evec1) const
{
  ...
  T absM00 = std::abs(m00);
  T absM01 = std::abs(m01);
  T absM11 = std::abs(m11);
  T maxAbsComp;
  if (absM00 >= absM11)
  {
