...
template <typename T>
RankFourTensorTempl<T>
RankTwoTensorTempl<T>::positiveProjectionEigenDecomposition() const
{
  // The calculate of projection tensor follows
  // C. Miehe and M. Lambrecht, Commun. Numer. Meth. Engng 2001; 17:337~353

  RankTwoTensorTempl<T> D, Q;
  this->diagonalize(Q, D);
  ...
  for (unsigned int i = 0; i < N; ++i)
  {
    epos[i] = (std::abs(D(i, i)) + D(i, i)) / 2.0;
    d[i] = D(i, i) > 0 ? 1.0 : 0.0;
  }
  ...
  {
    Ma.vectorOuterProduct(Q.column(a), Q.column(a));
  ...
  for (unsigned int a = 0; a < N; ++a)
    ...
    {
      Ma.vectorOuterProduct(Q.column(a), Q.column(a));
      Mb.vectorOuterProduct(Q.column(b), Q.column(b));

      Gab = Ma.mixedProductIkJl(Mb) + Ma.mixedProductIlJk(Mb);
      Gba = Mb.mixedProductIkJl(Ma) + Mb.mixedProductIlJk(Ma);

      T theta_ab;
      if (!MooseUtils::absoluteFuzzyEqual(D(a, a), D(b, b)))
        theta_ab = 0.5 * (epos[a] - epos[b]) / (D(a, a) - D(b, b));
      else
    ...
    std::vector<DualReal> & eigvals, RankTwoTensorTempl<DualReal> & eigvecs) const
{
  ...
  for (unsigned m = N - 1; m > 0; m--)
    do
    {
      ...
      D.addIa(shift);
      eigvecs = eigvecs * Q;
    } while (std::abs(D(m, m - 1)) > eps && iter < 100);
