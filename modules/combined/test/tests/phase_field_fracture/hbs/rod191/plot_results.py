...
data = pd.read_csv("bubble_pressure_r0.25_gas50_por10_ext0_rod191.csv",header=None)
time = data[0]
pressure = data[1]/1e6
...
# ax.text(intersection.xy[0][0]+10, intersection.xy[1][0]+5, s)
...
ax.set_title("\n".join(wrap("Radius=0.25$\mu$m, Initial gas pressure = 50 MPa, Porosity = 10$\%$, External pressure = 0 MPa", 50)))
ax.set_xlim(left=0)

ax.minorticks_on()
plt.savefig('bubble_pressure_r0.25_gas50_por10_ext0_rod191.pdf', bbox_inches='tight');
plt.close(fig)
