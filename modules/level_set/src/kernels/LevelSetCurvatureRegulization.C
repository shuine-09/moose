...
defineADValidParams(LevelSetCurvatureRegulization,
                    ADKernel,
                    params.addRequiredCoupledVar("grad_level_set", "The level set variable.");
...
LevelSetCurvatureRegulization<compute_stage>::LevelSetCurvatureRegulization(
  ...
  : ADKernel<compute_stage>(parameters),
    _grad_c(adCoupledVectorValue("grad_level_set")),
...
LevelSetCurvatureRegulization<compute_stage>::computeQpResidual()
{
  ...
  if (_grad_c[_qp].norm_sq() > std::numeric_limits<Real>::epsilon())
  {
    s = _grad_c[_qp].norm();
    n = _grad_c[_qp] / s;
  }
  const Real h = _current_elem->hmax();
  return _test[_i][_qp] * _u[_qp] - _grad_test[_i][_qp] * (n - h * _grad_u[_qp]);
}
