...
#include "MeltPoolLength.h"

registerMooseObject("LevelSetApp", MeltPoolLength);

defineLegacyParams(MeltPoolLength);

InputParameters
MeltPoolLength::validParams()
{
...
MeltPoolLength::MeltPoolLength(const InputParameters & parameters)
...
void
MeltPoolLength::computeQpValue()
{
  if (_ls[_qp] < 0.5 && _f_l[_qp] > 0.99)
  {
    _max_value = std::max(_max_value, _q_point[_qp](0));

    _min_value = std::min(_min_value, _q_point[_qp](0));
  }
}

void
MeltPoolLength::threadJoin(const UserObject & uo)
{
  const MeltPoolLength & pps = static_cast<const MeltPoolLength &>(uo);
...
Real
MeltPoolLength::getValue()
{
  gatherMax(_max_value);

  gatherMin(_min_value);

  _length = _max_value - _min_value;
