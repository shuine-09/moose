...
registerMooseObject("LevelSetApp", MeltPoolLevelSetLocation);

defineLegacyParams(MeltPoolLevelSetLocation);

InputParameters
MeltPoolLevelSetLocation::validParams()
{
  InputParameters params = ElementUserObject::validParams();
  params.addClassDescription("Find laser location.");
  params.addRequiredParam<VariableName>(
      "level_set", "The name of level set variable used to represent the interface");
...
MeltPoolLevelSetLocation::MeltPoolLevelSetLocation(const InputParameters & parameters)
  : ElementUserObject(parameters),
    ...
    _level_set_var_number(_subproblem
                              .getVariable(_tid,
                                           parameters.get<VariableName>("level_set"),
...
void
MeltPoolLevelSetLocation::initialize()
{
  _location_x = 0;
  _location_y = 0;
  _location_z = 0;
...
void
MeltPoolLevelSetLocation::execute()
{
  bool cut_elem = false;
  bool at_laser = false;
...
void
MeltPoolLevelSetLocation::threadJoin(const UserObject & uo)
{
  const MeltPoolLevelSetLocation & ls_uo = static_cast<const MeltPoolLevelSetLocation &>(uo);
