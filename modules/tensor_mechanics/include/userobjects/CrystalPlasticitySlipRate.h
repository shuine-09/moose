...
class CrystalPlasticitySlipRate : public CrystalPlasticityUOBase
{
 public:
  ...
  virtual void calcSchmidTensor(RankTwoTensor & crysrot, std::vector<RankTwoTensor> & schmid_tensor) const = 0;
  
  virtual bool calcSlipRate(unsigned qp, Real dt, std::vector<RankTwoTensor> & schmid_tensor, std::vector<Real> & val) const = 0;

  virtual bool calcSlipRateDerivative(unsigned qp, Real dt, std::vector<RankTwoTensor> & schmid_tensor, std::vector<Real> & val) const = 0;
