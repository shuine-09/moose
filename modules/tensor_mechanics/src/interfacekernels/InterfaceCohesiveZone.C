...
InterfaceCohesiveZone::InterfaceCohesiveZone(const InputParameters & parameters)
  : InterfaceKernel(parameters),
    ...
    _max_normal_separation(getMaterialPropertyOld<Real>("max_normal_separation")),
    _normal_separation(getMaterialProperty<Real>("normal_separation"))
{
...
InterfaceCohesiveZone::computeQpResidual(Moose::DGResidualType type)
{
  ...
  const Real max_normal_separation = _max_normal_separation[_qp];

  // Calculating Rotation matrix (from local to global coordinates):
  Real theta = std::atan2(_normals[_qp](0), _normals[_qp](1));
  ...
  Real delta_m_n = _normal_separation[_qp];
  ...
  {
    std::cout << "max_normal_separation >= delta_0 && max_normal_separation < delta_f" << std::endl;
    if (delta_m_n < 0.0)
...
InterfaceCohesiveZone::computeQpJacobian(Moose::DGJacobianType type)
{
  ...
  // Point interface_tangent(_normals[_qp](1), -_normals[_qp](0), 0);
  //
  ...
  if (_component == 0)
    dseparation_du = R[1][0] * _normals[_qp](0);
  else if (_component == 1)
    dseparation_du = R[0][0] * _normals[_qp](1);
  else
    mooseError("XFEMCohesiveConstraint: 3D is not supported.");
