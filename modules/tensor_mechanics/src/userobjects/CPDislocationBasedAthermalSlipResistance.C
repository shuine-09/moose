...
registerMooseObject("TensorMechanicsApp", CPDislocationBasedAthermalSlipResistance);

template <>
InputParameters
validParams<CPDislocationBasedAthermalSlipResistance>()
{
  ...
  params.addClassDescription("Dislocation based constitutive mode userobject class for athermal "
                             "slip resistance.  Override the virtual functions in your class");
  return params;
}

CPDislocationBasedAthermalSlipResistance::CPDislocationBasedAthermalSlipResistance(
    ...
    _latent_harden(getParam<Real>("rho_latent_hard_factor"))
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
    for (unsigned int j = 0; j < _variable_size; ++j)
    {
      ...
      if (i == j)
        _interaction_matrix[i * _variable_size + j] = _self_harden;
    }
}

bool
CPDislocationBasedAthermalSlipResistance::calcSlipResistance(unsigned int qp,
                                                             std::vector<Real> & val) const
{
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    Real disloc = 0.0;
    for (unsigned int j = 0; j < _variable_size; ++j)
      disloc += _interaction_matrix[i * _variable_size + j] *
                (_mat_prop_mobile_dislocation_density[qp][j] +
                 _mat_prop_immobile_dislocation_density[qp][j]);
    disloc *= _q_p;
