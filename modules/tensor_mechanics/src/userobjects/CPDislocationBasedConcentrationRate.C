...
template <> InputParameters validParams<CPDislocationBasedConcentrationRate>() {
...
CPDislocationBasedConcentrationRate::CPDislocationBasedConcentrationRate(
    ...
    : CrystalPlasticityStateVarRateComponent(parameters),
      ...
      _flow_direction(getMaterialProperty<std::vector<RankTwoTensor>>(
          parameters.get<std::string>("uo_climb_rate_name") +
          "_flow_direction")),
...
bool CPDislocationBasedConcentrationRate::
  ...
  Real pre_factor =
      2.0 * libMesh::pi * _diffusivity / (_b * std::log(rinf / _rc));

  val[0] = 0.0;

  for (unsigned int i = 0; i < size; ++i) {
    Real ceq =
        _c0 * std::exp(-tau(i) * _molar_volume / (_gas_constant * _temp));
    Real vc =
        pre_factor * (ceq - _cv[qp][0]) * (_rho_m[qp][i] + _rho_i[qp][i]) * _b;
    val[0] += vc;
    // std::cout << "ceq = " << ceq << ", cv = " << _cv[qp][0] << ", vc = " <<
    // vc << ", tau = " << tau(i) << std::endl;
  }
