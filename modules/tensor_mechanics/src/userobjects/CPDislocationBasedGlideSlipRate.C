...
registerMooseObject("TensorMechanicsApp", CPDislocationBasedGlideSlipRate);

template <>
InputParameters
validParams<CPDislocationBasedGlideSlipRate>()
{
  ...
  params.addParam<std::string>("uo_thermal_slip_resistance_name",
                               "Name of slip resistance property: Same as slip resistance user "
                               "object specified in input file.");
  params.addParam<std::string>("uo_athermal_slip_resistance_name",
                               "Name of slip resistance property: Same as slip resistance user "
                               "object specified in input file.");
...
CPDislocationBasedGlideSlipRate::CPDislocationBasedGlideSlipRate(const InputParameters & parameters)
  : CrystalPlasticitySlipRate(parameters),
    ...
    _mat_prop_thermal_slip_resistance(getMaterialProperty<std::vector<Real>>(
        parameters.get<std::string>("uo_thermal_slip_resistance_name"))),
    _mat_prop_athermal_slip_resistance(getMaterialProperty<std::vector<Real>>(
        parameters.get<std::string>("uo_athermal_slip_resistance_name"))),
...
void
CPDislocationBasedGlideSlipRate::calcFlowDirection(
...
bool
CPDislocationBasedGlideSlipRate::calcSlipRate(unsigned int qp,
                                              Real dt,
                                              std::vector<Real> & val) const
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    ...
    Real c = (std::abs(tau(i)) - _mat_prop_athermal_slip_resistance[qp][i]) /
             _mat_prop_thermal_slip_resistance[qp][i];
    ...
    // std::cout << "glide rate [ " << i << "] = " << val[i] << ", prefactor = " << _prefactor <<
    // ", a = " << a << ", c2 = " << c2 << ", b = " << b << ", sgn_tau = " << sgn_tau <<
    // std::endl;
  }
...
bool
CPDislocationBasedGlideSlipRate::calcSlipRateDerivative(unsigned int qp,
                                                        Real dt,
                                                        std::vector<Real> & val) const
{
  DenseVector<Real> tau(_variable_size);
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    ...
    val[i] = -_prefactor * a * b * std::exp(-b * c2) * sgn_tau * dc2_dc * dc_dtau;
  }
