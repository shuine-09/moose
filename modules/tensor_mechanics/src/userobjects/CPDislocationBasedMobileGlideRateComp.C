...
InputParameters validParams<CPDislocationBasedMobileGlideRateComp>() {
  ...
  params.addParam<Real>("rho_mult_factor", 0.143,
                        "Dislocation multiplication factor");
  params.addParam<Real>("rho_m_capture_radius", 1e-6,
                        "Capture radius for mutual annihilation of mobile "
                        "dislocations: Unit in mm");
  params.addParam<Real>("rho_imm_factor", 0.36,
                        "Immobilization factor due to dislocations");
...
CPDislocationBasedMobileGlideRateComp::CPDislocationBasedMobileGlideRateComp(
    ...
    : CrystalPlasticityStateVarRateComponent(parameters),
      _mat_prop_mobile_dislocation_density(
          getMaterialProperty<std::vector<Real>>(parameters.get<std::string>(
              "uo_mobile_dislocation_density_name"))),
      _mat_prop_immobile_dislocation_density(
          getMaterialProperty<std::vector<Real>>(parameters.get<std::string>(
              "uo_immobile_dislocation_density_name"))),
...
bool CPDislocationBasedMobileGlideRateComp::
    calcStateVariableEvolutionRateComponent(unsigned int qp,
  ...
  for (unsigned int i = 0; i < _variable_size; ++i) {
    d_self = _k_mul * std::sqrt(tot_rho_m) *
             std::abs(_mat_prop_glide_slip_rate[qp][i]) / _b;
    d_ann = 2.0 * _r_c * _mat_prop_mobile_dislocation_density[qp][i] *
            std::abs(_mat_prop_glide_slip_rate[qp][i]) / _b;

    lambda_inv =
        _beta_rho * std::sqrt(_mat_prop_mobile_dislocation_density[qp][i] +
