...
registerMooseObject("TensorMechanicsApp", CPDislocationDensityRatePptResistance);

template <>
InputParameters
validParams<CPDislocationDensityRatePptResistance>()
{
  ...
  params.addParam<std::string>("property_in_function_name",
                               "Property name used in function to evaluate property");
...
CPDislocationDensityRatePptResistance::CPDislocationDensityRatePptResistance(
  ...
  : CrystalPlasticityStateVarRateComponent(parameters),
    ...
    _mat_prop(getMaterialProperty<std::vector<Real>>(
        parameters.get<std::string>("property_in_function_name"))),
...
bool
CPDislocationDensityRatePptResistance::calcStateVariableEvolutionRateComponent(
    unsigned int qp, std::vector<Real> & val) const
{
  Point p;
  for (unsigned int i = 0; i < _variable_size; ++i)
    val[i] = _factor_function->value(_mat_prop[qp][i], p) * std::sqrt(_number_density * _size) *
             std::abs(_mat_prop_rate[qp][i]);
