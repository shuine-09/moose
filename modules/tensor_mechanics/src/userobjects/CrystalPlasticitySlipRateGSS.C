...
void
CrystalPlasticitySlipRateGSS::calcFlowDirection(unsigned int qp, std::vector<RankTwoTensor> & flow_direction, unsigned int op_index) const
{
  ...
  for (unsigned int i = 0; i < _variable_size; ++i)
  {
    for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
    {
      ...
      for (unsigned int k = 0; k < LIBMESH_DIM; ++k)
        mo(i*LIBMESH_DIM+j) = mo(i*LIBMESH_DIM+j) + _crysrot[qp][op_index](j,k) * _mo(i*LIBMESH_DIM+k);
    }
    ...
    for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
    {
      ...
      for (unsigned int k = 0; k < LIBMESH_DIM; ++k)
        no(i*LIBMESH_DIM+j) = no(i*LIBMESH_DIM+j) + _crysrot[qp][op_index](j,k) * _no(i*LIBMESH_DIM+k);
    }
  }
