...
Elem * XFEM::getElemFromUniqueID(unique_id_type uid)
{
...
XFEM::markCutEdgesByState(Real time)
{
       ...
       ++pmeit)
  {
    ...
    std::cout << "WJ : find_compatible_direction " << find_compatible_direction << ", edge cut = " << edge_cut << std::endl;
  ...
  for (pmeit = _elem_crack_origin_direction_map.begin(); pmeit != _elem_crack_origin_direction_map.end(); ++pmeit)
  {
    ...
    if (edge_cut)
    {
      ...
      else
      {
        ...
        Real x0 = crack_tip_origin(0);
        Real y0 = crack_tip_origin(1);
        Real crack_growth_increment = _crack_growth_increment; //0.0002; //0.00025; // !!!!! 0.0001 TEST ONLY 
...
bool
XFEM::cutMeshWithEFA(NonlinearSystemBase & nl,
                     AuxiliarySystem & aux)
{
  ...
  for (unsigned int i = 0; i < NewElements.size(); ++i)
  {
    ...
    for (unsigned int j = 0; j < NewElements[i]->numNodes(); ++j)
    {
      ...
      if (parent_elem->is_semilocal(_mesh->processor_id()))
      {
        ...
        {
          saveSolutionForNode(libmesh_node, solution_node, nl,
                              _new_node_solution, current_solution, old_solution);
          saveSolutionForNode(libmesh_node, solution_node, aux,
                              _new_node_aux_solution, current_aux_solution, old_aux_solution);
    ...
    if (parent_elem->processor_id() == _mesh->processor_id())
    {
      ...
      saveSolutionForElement(libmesh_elem, parent_elem, nl,
                             _new_elem_solution, current_solution, old_solution);
      saveSolutionForElement(libmesh_elem, parent_elem, aux,
                             _new_elem_aux_solution, current_aux_solution, old_aux_solution);
    }
                          ...
                          const NumericVector<Number> & old_solution)
{
  ...
  for (auto var : vars)
  {
    if (var->isNodal())
    {
      ...
      set_intersection(var_subdomains.begin(), var_subdomains.end(), sids.begin(), sids.end(),
                      ...
                      NumericVector<Number> & old_solution)
{
  ...
  for (auto nit = stored_solution.begin();
       nit != stored_solution.end(); ++nit)
  {
    _stored_solution_dofs_scratch.clear();
    ...
    for (unsigned int i = 0; i < _stored_solution_dofs_scratch.size(); ++i)
    {
      ...
      old_solution.set(_stored_solution_dofs_scratch[i], node_solution[_stored_solution_dofs_scratch.size() + i]);
    }
  }
}
                         ...
                         NumericVector<Number> & old_solution)
{
  ...
  for (auto eit = stored_solution.begin();
       eit != stored_solution.end(); ++eit)
  {
    _stored_solution_dofs_scratch.clear();
    ...
    for (unsigned int i = 0; i < _stored_solution_dofs_scratch.size(); ++i)
    {
      ...
      old_solution.set(_stored_solution_dofs_scratch[i], elem_solution[_stored_solution_dofs_scratch.size() + i]);
    }
  }
}
