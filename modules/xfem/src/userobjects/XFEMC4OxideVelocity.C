...
XFEMC4OxideVelocity::XFEMC4OxideVelocity(
...
Real
XFEMC4OxideVelocity::computeMovingInterfaceVelocity(
...
//                  (value_positive - value_negative + _equilibrium_concentration_jump));
  ...
  const Real J_v = mobil_v * potential * ( con_v_ox_w - con_v_ox_m * pow( eta, 2 ) ) / ( 1 - pow( eta, 2 ) ) / 1e-5; //( _initial_cut - _cut_data[0] );
  const Real J_o = zircaloy_density * Na / (zirconium_atmass * 1e-3) * _diffusivity_at_positive_level_set * grad_negative(0);
