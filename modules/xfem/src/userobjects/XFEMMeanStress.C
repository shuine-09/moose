...
registerMooseObject("XFEMApp", XFEMMeanStress);

template <>
InputParameters
validParams<XFEMMeanStress>()
{
...
XFEMMeanStress::getStressTensor()
{
  ...
  for (unsigned int i = 0; i < _num_crack_front_points; i++)
  {
    ...
    {
      Real flag = _xfem->flagQpoint(
          undisplaced_elem, _q_point[qp]); // qp inside (flag = 1) or ouside (flag = 0) real domain
      Point dist_to_crack_front_vector = _q_point[qp] - crack_front;
      Real dist = std::pow(dist_to_crack_front_vector.size_sq(), 0.5);
      Real fact = 1.0 / (pow(2 * libMesh::pi, 1.5) * pow(_radius, 3.0)) *
                  std::exp(-0.5 * pow(dist / _radius, 2.0));
      ...
      if (dist < 2.0 * _radius && flag > 0.5)
      {
        ...
        if (dist < _weibull_radius && flag > 1.5)
        {
          ...
          {
            // std::cout << "weillbull at tip = " << _weibull_at_tip[i] << ", weibull_eta[qp] = " <<
            // _weibull[qp] << std::endl;
...
XFEMMeanStress::finalize()
{
  ...
  for (unsigned int i = 0; i < _num_crack_front_points; i++)
  {
    ...
    // std::cout << "stress = " << tensor_quantity << ", weibull_tip = " << _weibull_at_tip[i] << "
    ...
    // std::cout << "crack front index (" << i << ") : average stress direction  = " << normal <<
    // std::endl;
  }
}
