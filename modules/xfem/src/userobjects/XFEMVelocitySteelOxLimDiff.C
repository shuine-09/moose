...
#include "XFEMVelocitySteelOxLimDiff.h"

registerMooseObject("XFEMApp", XFEMVelocitySteelOxLimDiff);

template <>
InputParameters
validParams<XFEMVelocitySteelOxLimDiff>()
{
  InputParameters params = validParams<XFEMMovingInterfaceVelocityBase>();
  params.addParam<Real>("diffusivity_Mn_steel",0.036,"Diffusivity of Mn in the steel");
  params.addClassDescription(
      "Calculate the oxide/gas interface velocity for the 1 oxide layer (spinel)  model for steel corrosion.");
  return params;
}

XFEMVelocitySteelOxLimDiff::XFEMVelocitySteelOxLimDiff(const InputParameters & parameters)
  : XFEMMovingInterfaceVelocityBase(parameters),
  _diffusivity_Mn_steel(getParam<Real>("diffusivity_Mn_steel"))
{
}

Real
XFEMVelocitySteelOxLimDiff::computeMovingInterfaceVelocity(unsigned int point_id) const
{
...
//  const Real C_steel = rho_steel * Na / M_steel;
  ...
  //const Real E_m_VMn(0.6);     // Mn vacancies energy of migration [eV]
  ...
  //std::cout<< "V_spinel: " << V_spinel << std::endl;
  std::cout<< "grad_neg: " << grad_negative(0) << std::endl;
  //std::cout<< "grad_pos: " << grad_positive(0) << std::endl;
  //std::cout<< "val_neg: " << value_negative << std::endl;
  //std::cout<< "val_pos: " << value_positive << std::endl;
  //std::cout<< "D : " << _diffusivity_Mn_steel << std::endl;
  const Real v_ox = - 1e9 * _diffusivity_Mn_steel * grad_negative(0) * V_spinel;  // e-9 because kept C in /nm³ but want v_ox in µm/hr
