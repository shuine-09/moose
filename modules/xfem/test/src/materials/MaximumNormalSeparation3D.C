...
validParams<MaximumNormalSeparation3D>()
{
  ...
  params.addRequiredCoupledVar("disp_z", "Name of the variable to couple");
...
void
MaximumNormalSeparation3D::resetQpProperties()
{
  _max_normal_separation[_qp] = 0.0;
}

void
MaximumNormalSeparation3D::computeQpProperties()
{
  ...
  for (const auto & it : *element_pair_locators)
  {
    ...
    if (findIter != elem_pairs.end())
    {
      ...
      normal_distance = interface_normal(0) * (_disp_x_neighbor[_qp] - _disp_x[_qp]) +
                        ...
                        interface_normal(2) * (_disp_z_neighbor[_qp] - _disp_z[_qp]);
    }
...
void
MaximumNormalSeparation3D::initQpStatefulProperties()
{
  _max_normal_separation[_qp] = 0.0;
}
